...
build_json() {
  ...
  NETDIAG_LINK_METERED_CERTAIN="${LINK_METERED_CERTAIN:-0}" \
  ...
  NETDIAG_BROWSER_DESYNC_COUNT="${BROWSER_DESYNC_COUNT:-0}" \
  NETDIAG_BROWSER_DESYNC_APP="${BROWSER_DESYNC_APP:-}" \
  NETDIAG_BROWSER_DESYNC_RUNNING_VER="${BROWSER_DESYNC_RUNNING_VER:-}" \
  NETDIAG_BROWSER_DESYNC_DISK_VER="${BROWSER_DESYNC_DISK_VER:-}" \
  NETDIAG_BROWSER_DESYNC_PID="${BROWSER_DESYNC_PID:-}" \
  ...
  NETDIAG_IPV6_ONLY="${IPV6_ONLY:-0}" \
  NETDIAG_IPV6_CLAT="${IPV6_CLAT:-0}" \
  ...
  NETDIAG_PATH_PRIVATE_RELAY="${PATH_PRIVATE_RELAY:-0}" \
  NETDIAG_PATH_ENCRYPTED_DNS="${PATH_ENCRYPTED_DNS:-0}" \
  NETDIAG_PATH_ENCRYPTED_DNS_SERVER="${PATH_ENCRYPTED_DNS_SERVER:-}" \
  ...
  NETDIAG_WIFI_DISCONNECT_LINES="${WIFI_DISCONNECT_LINES:-}" \
  NETDIAG_WIFI_PRIVILEGED="${WIFI_PRIVILEGED:-0}" \
  NETDIAG_WIFI_NAME_HIDDEN="${WIFI_NAME_HIDDEN:-0}" \
  ...
  NETDIAG_ARP_ACTIVE_COUNT="$ARP_ACTIVE_COUNT" \
  ...
  NETDIAG_AV_MEASURED="${AV_MEASURED:-0}" \
  NETDIAG_AV_WINDOW_HOURS="${AV_WINDOW_HOURS:-}" \
  NETDIAG_AV_OUTAGE_COUNT="${AV_OUTAGE_COUNT:-}" \
  NETDIAG_AV_DOWNTIME_S="${AV_DOWNTIME_S:-}" \
  NETDIAG_AV_LONGEST_S="${AV_LONGEST_S:-}" \
  NETDIAG_AV_FLAP_COUNT="${AV_FLAP_COUNT:-}" \
  NETDIAG_AV_UNOBSERVED_PCT="${AV_UNOBSERVED_PCT:-}" \
  NETDIAG_TRAFFIC_MEASURED="${TRAFFIC_MEASURED:-0}" \
  NETDIAG_TRAFFIC_DOWN_MBPS="${TRAFFIC_DOWN_MBPS:-}" \
  NETDIAG_TRAFFIC_UP_MBPS="${TRAFFIC_UP_MBPS:-}" \
  NETDIAG_TRAFFIC_SAMPLED_S="${TRAFFIC_SAMPLED_S:-}" \
  NETDIAG_TRAFFIC_TOP_JSON="${TRAFFIC_TOP_JSON:-[]}" \
  NETDIAG_WATCHER_INSTALLED="${WATCHER_INSTALLED:-0}" \
  NETDIAG_WATCHER_STATE="${WATCHER_STATE:-absent}" \
  NETDIAG_WATCHER_PROGRAM="${WATCHER_PROGRAM:-}" \
  NETDIAG_WATCHER_PLIST_INTERVAL_S="${WATCHER_PLIST_INTERVAL_S:-}" \
  NETDIAG_WATCHER_LAST_EXIT="${WATCHER_LAST_EXIT:-}" \
  NETDIAG_WATCHER_HEARTBEAT_AGE_S="${WATCHER_HEARTBEAT_AGE_S:-}" \
  NETDIAG_WATCHER_INSTALLED_AGE_S="${WATCHER_INSTALLED_AGE_S:-}" \
  NETDIAG_WATCHER_PATH_BLOCKED="${WATCHER_PATH_BLOCKED:-0}" \
