...
enum BinaryLocator {
    ...
    static func environment() -> [String: String] {
        ...
        // demonstrably share ~/hopwatch rather than probably sharing it.
        ...
        if let ssid = liveSSID() {
            env["HOPWATCH_SSID_HINT"] = ssid
            env["NETDIAG_SSID_HINT"] = ssid
        }
        return env
    }
    ...
    /// `cli/bin/hopwatch` inside it, so hitting this message means the
    ...
    static let missingBinaryMessage = """
        Hopwatch couldn't find the hopwatch command-line tool. \
