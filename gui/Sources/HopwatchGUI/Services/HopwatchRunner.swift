...
enum HopwatchError: LocalizedError {
    case binaryNotFound
    ...
    var errorDescription: String? {
        ...
        case .scriptError(let s):  return "hopwatch couldn't complete the check: \(s)"
        case .badJSON(let s):      return "hopwatch returned something unreadable: \(s)"
        ...
        case .cliTooOld(let found, let needs):
            ...
            guard let found else {
                return """
                    Your hopwatch command doesn't answer this app's version check — \
                    this app needs \(AppVersion.phrase(for: needs)) or newer. \(fix)
                    """
            }
            if found == needs {
                return "The override reports an unexpected version format. \(fix)"
            }
            return """
                Your hopwatch command is too old (\(found)) — this app \
                needs \(AppVersion.phrase(for: needs)) or newer. \(fix)
                """
        }
    }
}

typealias NetdiagError = HopwatchError
typealias NetdiagRunner = HopwatchRunner

/// Runs `hopwatch --json` and decodes the result.
///
...
/// criterion 2, `hopwatch --json | jq .`, and it is why there is no partial
