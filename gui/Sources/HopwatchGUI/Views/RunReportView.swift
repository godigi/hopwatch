...
struct RunReportView: View {
    ...
    private func copyShareableReport() {
        guard let raw = rawJSON else {
            shareError = "This report came from an older hopwatch and can't be shared."
            return
        }
        Task { @MainActor in
            isSharing = true
    ...
    private func saveMarkdownReport() {
        ...
        Task { @MainActor in
            ...
            defer { isSharing = false }
            do {
                let text = try await HopwatchRunner.share(rawJSON: raw)
    ...
    private func saveJSONReport() {
        ...
        Task { @MainActor in
            ...
            defer { isSharing = false }
            do {
                let json = try await HopwatchRunner.shareJSON(rawJSON: raw)
