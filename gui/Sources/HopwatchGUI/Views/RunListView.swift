...
struct RunListView: View {
    ...
    /// the id `--history` stamps on it, and a hopwatch from before that
    ...
    private var unopenableNotice: some View {
        Label {
            Text("\(unopenableCount) of these check\(unopenableCount == 1 ? "" : "s") can't be opened: the hopwatch command line on this Mac is older than this app and didn't record an id for them. Updating hopwatch makes them openable.")
                .font(.caption)
                .fixedSize(horizontal: false, vertical: true)
        } icon: {
